  ...
  <div>
    <div>
      <div class="headers">
        <!-- //发布学院 -->
        <div class="headers">
          <div class="product-type">采购账单来源：</div>
          <div>
            <template>
    ...
    <el-table :data="tableData" :key="tableData.id" style="width: 100%;font-size: 15px;" stripe>
      ...
      <el-table-column prop="company_name" label="中标公司名称"> </el-table-column>
      <el-table-column prop="reality_price" label="成交价格"> </el-table-column>

      <el-table-column align="right" label="操作1">
        <template slot-scope="scope">
          <el-button size="mini" @click="handleEdit(scope.row)">详情</el-button>
        </template>
      </el-table-column>
         <el-table-column align="right" label="操作2">
        <template slot-scope="scope">
         <el-button
            size="mini"
            type="danger"
...
<script>
//必须有这个模块
import {purchaseListManageJs} from  "./purchaseListManage";
export default {
  data() {
    return {
      //某某年的消费钱
  ...
  methods: {
    ...
    async search() {
      purchaseListManageJs.gettDetail(this.selects).then((result) => {
      ...
      if (res.success) {
        ...
        this.handleList();
        this.$message.success("查询到"+this.tableData.length+"条数据");
      } else {
        console.log("purchasingList请求失败！");
        this.$message.success("查询出错");
      }
    },
    handleList() {
      ...
      for (let i = 0; i < this.purchaseList.length; i++) {
        ...
        //初始化时，有的商品还没有结果
        this.tableData[i].reality_price = this.resultList[
          this.purchaseList[i].id
        ].reality_price;
